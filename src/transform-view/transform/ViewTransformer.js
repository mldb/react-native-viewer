...
export default class ViewTransformer extends Component {
  ...
  onResponderMove(evt, gestureState) {
    ...
    if (gestureState.previousPinch && gestureState.pinch && this.props.enableScale) {
      ...
      transform.translateX = this.state.scale == 1 ? 0 : this.state.translateX + dx / this.state.scale
  ...
  onResponderRelease(evt, gestureState) {
    ...
    } else if (gestureState.singleTapUp && !gestureState.doubleTapUp) {
      setTimeout(() => {
        if (!this.doubled) {
          this.singled = true
